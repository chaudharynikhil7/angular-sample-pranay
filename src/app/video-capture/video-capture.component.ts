...
export class VideoCaptureComponent {
  ...
  async ngAfterViewInit(): Promise<void> {
    try {
      ...
      const cameraView: any = await CameraView.createInstance();
      cameraView.setUIElement(document.getElementById('div-ui-container'));
      ...
      this.cvRouter.addResultReceiver({
        onDecodedBarcodesReceived: (result) => {
          ...
          // this.scanCodeService.scanResultData$.next(scanResult)
          // this.route.navigate( [this.scanCodeService.afterScanRedirect])
        },
      });
